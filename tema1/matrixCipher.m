function [] = matrixCipher()
	...
	file_matrice = fopen("key1A", "r");
	file_string = fopen("input1A", "r");
	file_write = fopen("output1A", "w");
	...
	for i = 1:new_d/n %new_d / n inmultiri cu matricea
		...
		B = V( (i - 1) * n + 1: i * n) * A; % selectez cele n elemente care ma intereseaza si inmultesc cu matricea A
		B = mod(B, 29);
