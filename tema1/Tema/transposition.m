function [ ] = transposition()
	...
	if(mod(m, n)) % bag zerouri doar cand e nevoie
	...
	m = columns(C);
	ct = 1;
	ok = 0; % pentru a afisa matricea de translatie o singura data
	for i = 1: m / n
	
		D = C((i - 1) * n  + 1: i * n);

		R = eye(n + 1); % construiesc matricea de translatie, de dimesniunea (n + 1) x (n + 1)
		...
		if(i * n > ct * 1000 && (i - 1) * n <= ct * 1000) % aici formez o matrice care translateaza unele elemente cu k, iar altele cu k + 1, 
			% asta se intampla doar cand indicele unori blocuri de n elemente e mai mic ca un multiplu de 1000 si altele mai mare 
